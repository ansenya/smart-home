...
bool ensureMqttConnected(){
  ...
  bool ok = mqtt.publish(descTopic.c_str(), out.c_str(), true);
  LOG("[MQTT] describe size=%d topic=%d", out.length(), strlen(descTopic.c_str()));
  LOG("[MQTT] describe -> %s : %s", descTopic.c_str(), ok ? "OK" : "FAILED");
