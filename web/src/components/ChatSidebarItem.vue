  ...
  <div class="item" :class="{ 'item--active': active }" @click="emit('click')">
    ...
    <div class="item-body">
      <input
        ...
        @click.stop
      />
      <span v-else class="item-title">{{ chat.title || 'Новый чат' }}</span>
      <span class="item-date">{{ formatDate(chat.created_at) }}</span>
    </div>

    <div v-if="!editing" class="item-actions">
      <button class="action-btn" title="Переименовать" @click.stop="startEdit">
        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/>
        </svg>
      </button>
      <button class="action-btn action-btn--danger" title="Удалить" @click.stop="emit('delete')">
        <svg width="12" height="12" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
